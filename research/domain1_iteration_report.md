...
## 十一、Survey Agent B 补充数据（前向/反馈切换边界精化）

### 关键时序精化（Flanagan & Wing 1997; Desmurget & Grafton 2000）

| 时间窗口 | 主导控制 | 量化数值 |
|---------|---------|---------|
| 0–100 ms（预负荷+负荷早期） | **纯前馈控制** | 握力领先负载力 **+10–14 ms**（预测性） |
| ~100–150 ms | **前向模型辅助快速修正** | 基于预测误差的早期修正启动 |
| ~150–200 ms+ | **外周感觉反馈接管** | 触觉信号到达皮层后的反应性修正 |
| >200 ms（静态期） | **反馈维持 + 前向模型持续更新** | 双重控制并行 |

### 老化导致前向模型从预测性退化为反应性（Diermayr et al. 2007）

| 指标 | 青年组 | 老年组 | 差异 |
|-----|-------|-------|-----|
| 握力-负载力时序 | 握力**提前**负载力 **+7 ms** | 握力**滞后**负载力 **-26 ms** | 相差 33 ms：从预测→反应 |

**这是"老化使前向模型退化为反应性控制"的最强量化证据（★★★★★）**

### 完全去传入（IW）失去视觉后崩溃速度（Cole & Sedgwick 1992）

- 闭眼后 **~30 秒**内运动控制完全崩溃
- 仅依赖视觉时，运动速度降低约 **40–60%**

### 小脑患者接球实验（Nowak et al. 2004）

- 健康对照：握力峰值与负载力峰值**接近同步**
- 小脑患者：握力峰值**滞后**负载力峰值 **~66 ms**
- 冲击反应潜伏期：对照 **79 ms** vs 患者 **88 ms**

### 前向模型学习速率（Wolpert & Flanagan 2000；Flanagan et al. 2003）

- 前向模型（预测能力）：**1–2 次试验**即可习得（快速可塑）
- 逆向模型（控制能力）：需要**数十次**试验才趋稳
- 前向模型未使用时的衰减：**~24 小时衰减约 50%**

**对遥操作的含义**：操作者在第1–2次抓取时即可建立对物体重量的前向预测，但如果触觉回传延迟破坏了这个学习过程，则需要更多次试验才能达到相同熟练度
